...
class CrashReportsScreen(ModalScreen):
    ...
    def compose(self) -> ComposeResult:
        with Container(id="cr-root"):
            yield Label(" 🔴  Crash-репорты FlashSync — история падений", id="cr-header")
    ...
    def _load_list(self) -> None:
        ...
        self.query_one("#cr-status", Label).update(f"Найдено {len(self._reports)} репортов")
