...
Хранит последнее сканирование (план) чтобы при перезапуске не нужно
было сканировать заново. Также считает статистику плана по типам файлов.
...
from domain.models import SyncAction, ActionType, ProtectionLevel, FileInfo
...
class SessionManager:
    ...
    def save(self, plan: list[SyncAction], src_path: str, dst_path: str,
             scan_duration: float = 0.0) -> None:
        try:
            data = {
            ...
            # Сессия не критична для работы программы (это просто кэш последнего
            # скана), поэтому не показываем ошибку пользователю — но логируем,
            # иначе молчаливый сбой сохранения невозможно будет отличить от
            # "пользователь ещё не сканировал" при разборе багрепортов.
            _logger.debug(f"Не удалось сохранить сессию {self._profile_name}: {e}")
    ...
    def load(self, src_path: str, dst_path: str) -> Optional[tuple[list[SyncAction], str]]:
        try:
            ...
            if data.get("version") != SESSION_VERSION:
                return None
            if data.get("src_path") != src_path or data.get("dst_path") != dst_path:
                return None
            ...
            _logger.debug(f"Не удалось загрузить сессию {self._profile_name}: {e}")
            return None
    ...
    def clear(self) -> None:
        try:
            ...
            _logger.debug(f"Не удалось удалить файл сессии {self._profile_name}: {e}")
    ...
    def _fi_to_dict(self, fi: Optional[FileInfo]) -> Optional[dict]:
        if fi is None:
            return None
        return {
            "path": str(fi.path), "rel_path": fi.rel_path.as_posix(),
            "size": fi.size, "mtime": fi.mtime, "hash": fi.hash,
            "extension": fi.extension, "category": fi.category,
        }
    ...
    def _fi_from_dict(self, d: Optional[dict]) -> Optional[FileInfo]:
        ...
        try:
            return FileInfo(
                path=Path(d["path"]), rel_path=Path(d["rel_path"]),
                size=d["size"], mtime=d["mtime"], hash=d.get("hash"),
                extension=d.get("extension", ""), category=d.get("category", "other"),
            )
        except Exception as e:
            _logger.debug(f"Не удалось восстановить FileInfo из сессии: {e}")
            return None
    ...
    def _action_from_dict(self, d: dict) -> Optional[SyncAction]:
        try:
            ...
            _logger.debug(f"Не удалось восстановить SyncAction из сессии: {e}")
            return None


def plan_stats_by_category(plan: list[SyncAction]) -> dict[str, dict]:
    """Статистика плана по категориям файлов (image/video/document/...)."""
